...
var isOdd = tryRequire( resolve( __dirname, './../lib/native.js' ) );
var opts = {
	'skip': ( isOdd instanceof Error )
};
...
tape( 'the function returns `false` if provided `+-0`', opts, function test( t ) {
	t.strictEqual( isOdd( +0.0 ), false, 'returns expected value' );
	t.strictEqual( isOdd( -0.0 ), false, 'returns expected value' );
	t.end();
});

tape( 'WARNING: the function returns `true` if provided `+infinity`', opts, function test( t ) {
	t.strictEqual( isOdd( PINF ), true, 'returns expected value' );
	t.end();
});

tape( 'WARNING: the function returns `true` if provided `-infinity`', opts, function test( t ) {
	t.strictEqual( isOdd( NINF ), true, 'returns expected value' );
	t.end();
});

tape( 'the function returns `false` if provided `NaN`', opts, function test( t ) {
	t.strictEqual( isOdd( NaN ), false, 'returns expected value' );
	t.strictEqual( isOdd( 0.0/0.0 ), false, 'returns expected value' );
	t.end();
});
